...
namespace n_18_19
{
    class Program
    {
        static void Main()
        {
            ...
            if (new FileInfo("expproduct.dat").Length == 0)
            {
                products = LoadProductsFromFile("products.txt");//Список продуктов
            }
            else
            {
                using (FileStream fin = new FileStream("expproduct.dat", FileMode.Open))
                {
            ...
            //сериализация
            using (FileStream fout = new FileStream("expproduct.dat", FileMode.OpenOrCreate))
            {
